...
Stage implementations live in `simworld.stages`; each returns a list of output paths.
...
from simworld.tracking import Tracker, _git_head
...
def _state_dir(cfg: SimWorldConfig) -> Path:
    d = Path(cfg.paths.root) / ".stage_state"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _cfg_hash(cfg: SimWorldConfig, sections: list[str]) -> str:
...
def _cached(cfg: SimWorldConfig, name: str, sections: list[str]) -> list[str] | None:
...
def _save_state(cfg: SimWorldConfig, name: str, sections: list[str], outputs: list[str]) -> None:
    f = _state_dir(cfg) / f"{name}.json"
    f.write_text(json.dumps({"hash": _cfg_hash(cfg, sections), "outputs": outputs}, indent=2))


def run_pipeline(cfg: SimWorldConfig, tracker: Tracker) -> dict[str, object]:
    ...
    for name, sections in STAGE_ORDER:
        ...
        fn: Callable[[SimWorldConfig, Tracker], list[Path]] | None
