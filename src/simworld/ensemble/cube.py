...
from simworld.rules import backfire
...
try:  # Stage 10 (RL) runs in parallel with this stage; guard its registry.
    from simworld.agents.registry import load_policy
...
def cube_dir(cfg: SimWorldConfig) -> Path:
...
def resolve_policies(
    cfg: SimWorldConfig, names: Sequence[str]
    ...
    ``simworld.abm.policies.STATIC_POLICIES``. Learned policies (``rl_ppo``,
...
def _run_cells_ray(
    ...
    try:
        ...
        class _EmulatorActor:
            def __init__(self, cfg: SimWorldConfig, policy_names: Sequence[str]) -> None:
                self._cfg = cfg
...
def build_cube(
    cfg: SimWorldConfig, model: WorldModel, meta: dict[str, Any]
...
def run_ensemble(cfg: SimWorldConfig) -> EnsembleResult:
    """§10 Stage 11: build the scenario cube and cross-validate it against the ABM."""
    from simworld.ensemble.validation import run_validation
    from simworld.pipeline import Degraded
