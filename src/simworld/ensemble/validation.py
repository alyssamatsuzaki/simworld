...
not an input to :class:`~simworld.models.world_model.WorldModel` or to
:class:`~simworld.environments.emulator_env.EmulatorEnv` — the only thing that
...
from simworld.training.datamodule import load_theta_draws
...
def enforce_coverage_gate(
    cfg: SimWorldConfig, coverage: float, *, report_path: Path | None = None
) -> str:
...
def validation_dir(cfg: SimWorldConfig) -> Path:
...
def _degraded_report(cfg: SimWorldConfig, path: Path, note: str) -> ValidationReport:
...
def run_validation(
    cfg: SimWorldConfig, cube: pl.DataFrame, model: WorldModel, meta: dict[str, Any]
) -> ValidationReport:
