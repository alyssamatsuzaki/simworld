...
``SingleAgentView``
    A Gymnasium view over :class:`~simworld.environments.marl_env.RegulationMARLEnv`
...
``compare_c6``
    ...
    :func:`simworld.abm.model.strategic_controls_from_actions` maps exactly onto
    ``StrategicControls.neutral`` — i.e. the calibrated rule-based firms.

This module must never import :mod:`simworld.dgp` or read the answer key.
"""
...
from simworld.abm.policies import STATIC_POLICIES, levers_from_config
from simworld.agents.planning import RolloutStats
from simworld.environments.abm_env import ModelFactory
from simworld.environments.marl_env import AgentObservation, RegulationMARLEnv
from simworld.rules import PolicyLevers, QuarterOutcome, backfire
...
SCHEMA = "simworld.marl.c6_comparison.v1"
...
def reference_levers(cfg: SimWorldConfig) -> PolicyLevers:
...
def reference_book(cfg: SimWorldConfig) -> PolicyBook:
...
def marl_dir(cfg: SimWorldConfig) -> Path:
    return Path(cfg.paths.root) / "marl"


def comparison_path(cfg: SimWorldConfig) -> Path:
