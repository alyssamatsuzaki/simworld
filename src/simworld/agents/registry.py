...
def load_checkpoint_compat(cfg: SimWorldConfig) -> tuple[WorldModel, dict[str, Any]]:
...
def _ppo_checkpoint(cfg: SimWorldConfig) -> Path:
    return Path(cfg.paths.root) / "rl" / "ppo" / "model.zip"


def _dreamer_checkpoint(cfg: SimWorldConfig) -> tuple[Path, Path]:
...
def _random_policy(cfg: SimWorldConfig) -> PolicyFn:
...
def _ppo_policy(cfg: SimWorldConfig) -> PolicyFn:
...
def _dreamer_policy(cfg: SimWorldConfig) -> PolicyFn:
    from simworld.agents.dreamer import SquashedGaussianActor, normalize_obs, squash_action
...
def load_policy(cfg: SimWorldConfig, name: str) -> PolicyFn:
...
def available_policies(cfg: SimWorldConfig) -> list[str]:
