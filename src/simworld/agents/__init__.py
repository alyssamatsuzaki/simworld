...
SB3 PPO trained inside ``EmulatorEnv`` (:mod:`simworld.agents.ppo`, the
control group), and a latent Dreamer-style actor-critic trained purely on
imagined rollouts (:mod:`simworld.agents.dreamer`, the experiment).
:mod:`simworld.agents.registry` is the single lookup both this stage and the
Stage-11 ensemble use to turn a policy name into a callable, and
:mod:`simworld.agents.planning` rolls any such callable out in either world.
...
This package must never import :mod:`simworld.dgp` or read from the
...
from simworld.agents.dreamer import train_dreamer
...
def train_rl(cfg: SimWorldConfig) -> RlResult:
