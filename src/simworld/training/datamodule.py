...
from simworld.abm.model import ObservedWorld, load_observed_world
...
def aggregate_names(cfg: SimWorldConfig) -> list[str]:
    return [
        *AGGREGATE_BASE,
...
def aggregate_dim(cfg: SimWorldConfig) -> int:
    return len(AGGREGATE_BASE) + cfg.population.n_sectors + 10


def dataset_dir(cfg: SimWorldConfig) -> Path:
    return Path(cfg.paths.root) / "emulator" / "dataset"


def load_graph_bundle(cfg: SimWorldConfig) -> tuple[dict[str, torch.Tensor], Any]:
...
def load_theta_draws(cfg: SimWorldConfig) -> np.ndarray:
...
def initial_frame(cfg: SimWorldConfig, world: ObservedWorld) -> dict[str, np.ndarray]:
...
class EmulatorSequences:
    ...
    def __init__(self, cfg: SimWorldConfig, split: str = "train") -> None:
        import zarr
