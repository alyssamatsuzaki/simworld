...
from simworld.dgp.dynamics import Trajectory
from simworld.dgp.history import NEVER_TREATED
from simworld.dgp.world import THETA_STAR
from simworld.rules import FirmAttributes, SegmentAttributes
...
def aggregate_series(
    cfg: SimWorldConfig, traj: Trajectory, rng: np.random.Generator
) -> pl.DataFrame:
    rows = []
    s = cfg.dgp.sigma_obs
    for t in range(cfg.observed_quarters):
...
def market_stats(cfg: SimWorldConfig, traj: Trajectory, firms: FirmAttributes) -> pl.DataFrame:
    rows = []
    for t in range(cfg.observed_quarters):
...
def regime_p_full(
    cfg: SimWorldConfig, traj: Trajectory, firms: FirmAttributes, t_start: np.ndarray
) -> pl.DataFrame:
