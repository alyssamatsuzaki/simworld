...
from simworld.abm.policies import levers_from_config
from simworld.data import store
from simworld.data.schema import EDGES, validate_table
...
def _read_edges(cfg: SimWorldConfig, name: str) -> pl.DataFrame:
...
def _observed_graphs(cfg: SimWorldConfig, n_firms: int, n_segments: int) -> rules.Graphs:
...
def _observed_segments(
    cfg: SimWorldConfig, survey: pl.DataFrame, total_budget: float
...
def load_observed_world(cfg: SimWorldConfig, seed: int | None = None) -> ObservedWorld:
...
class RegulationModel(mesa.Model):
    ...
    def __init__(
        ...
        from simworld.abm.collect import make_data_collector
    ...
    def run(
        ...
        from simworld.abm.collect import firm_panel_from_collector, model_frame_from_collector
