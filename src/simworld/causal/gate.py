...
from simworld.causal.did import DidResult, estimate_did, group_time_att
from simworld.causal.estimate import PointEstimate, dml_onset
from simworld.causal.ground_truth import GroundTruthEffects, load_ground_truth
from simworld.data.ingest import read_panel_analysis
from simworld.data.store import read_observed
...
def load_calibrated_theta(cfg: SimWorldConfig) -> tuple[Theta, float, float]:
    """Posterior-mean Theta from Stage 4's combined posterior, plus (q0, q1)."""
    from simworld.calibration.posterior import posterior_mean_theta, posterior_path
...
def _treatment_schedule(
    cfg: SimWorldConfig, n_firms: int, seed: int
...
def simulate_interventions(
    cfg: SimWorldConfig, theta: Theta, *, n_seeds: int = 8
...
def run_gate(cfg: SimWorldConfig, *, n_sim_seeds: int = 8) -> FourNumbers:
...
def write_gate_outputs(cfg: SimWorldConfig, result: FourNumbers) -> list[Path]:
