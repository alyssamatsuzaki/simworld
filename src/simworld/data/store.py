...
`read_oracle` raises unless the caller is `simworld.evaluation`, a test, or a
...
from simworld.data.schema import ALL_OBSERVED, TableSpec, validate_table
from simworld.types import SimWorldConfig

_ORACLE_ALLOWED_FRAGMENTS = (
    "simworld/evaluation/",
    "simworld/data/generate.py",
    "simworld/causal/ground_truth.py",
    "/tests/",
    "site-packages/pytest",
)


def observed_dir(cfg: SimWorldConfig) -> Path:
    return Path(cfg.paths.data) / "observed"


def oracle_dir(cfg: SimWorldConfig) -> Path:
    return Path(cfg.paths.root) / "oracle"


def write_observed(
    cfg: SimWorldConfig, name: str, df: pl.DataFrame, spec: TableSpec | None = None
) -> Path:
    ...
    if spec is None:
        ...
        raise ValueError(
            ...
            "TableSpec to simworld.data.schema.ALL_OBSERVED or pass spec= explicitly."
        )
...
def read_observed(cfg: SimWorldConfig, name: str, validate: bool = True) -> pl.DataFrame:
...
def read_oracle(cfg: SimWorldConfig, name: str) -> Any:
    ...
    if not _caller_is_allowed():
        ...
        raise RuntimeError(
            f"oracle read from {offender!r}: only simworld.evaluation, tests, and "
...
def write_oracle_parquet(cfg: SimWorldConfig, name: str, df: pl.DataFrame) -> Path:
    d = oracle_dir(cfg)
    d.mkdir(parents=True, exist_ok=True)
    path = d / f"{name}.parquet"
    df.write_parquet(path, compression="snappy")
...
def write_oracle_json(cfg: SimWorldConfig, name: str, payload: dict[str, Any]) -> Path:
    d = oracle_dir(cfg)
    d.mkdir(parents=True, exist_ok=True)
    path = d / f"{name}.json"
