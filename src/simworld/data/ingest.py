...
from simworld.data.schema import EDGES, FIRM_PANEL, PANEL_ANALYSIS, validate_table
...
def _read_real_panel(cfg: SimWorldConfig) -> pl.DataFrame:
...
def _read_observed_edges(cfg: SimWorldConfig, name: str) -> pl.DataFrame:
...
def _read_supply_edges(cfg: SimWorldConfig) -> pl.DataFrame:
...
def ingest(cfg: SimWorldConfig) -> Path:
    panel = (
...
def read_panel_analysis(cfg: SimWorldConfig) -> pl.DataFrame:
    df = pl.read_parquet(Path(cfg.paths.data) / "panel_analysis.parquet")
    validate_table(df, PANEL_ANALYSIS)
    return df


def observation_sanity(cfg: SimWorldConfig) -> dict[str, float]:
