...
This module is a WORLD BUILDER: it may import simworld.dgp (see the firewall
...
from simworld.data.schema import EDGES
from simworld.dgp import history as dgp_history
from simworld.dgp import observation as obs
from simworld.dgp.dynamics import Trajectory, run_dgp
from simworld.dgp.world import (
    CONSTANTS,
    THETA_STAR,
    generate_firms,
    generate_segments,
...
from simworld.graphs.build import RegGraphs, build_graphs, edges_frame
...
def _write_graph_edges(cfg: SimWorldConfig, reg: RegGraphs) -> list[Path]:
    ...
    for name, pairs in observed.items():
        df = _edges_df(pairs)
        from simworld.data.schema import validate_table
...
def _do_interventions(
    ...
    from simworld.causal.did import group_time_att
...
def generate_ground_truth(cfg: SimWorldConfig) -> GenerationResult:
