...
it as ``simworld.visualization.dashboard`` (or ``scripts.dashboard``) for
...
from simworld.visualization._io import action_bounds, load_json
...
def load_default_config(profile: str = "smoke") -> SimWorldConfig:
...
def _fallback_train_actions(cfg: SimWorldConfig, n_samples: int = 512) -> np.ndarray:
...
def _train_action_distribution(cfg: SimWorldConfig) -> np.ndarray:
    ...
    """
    try:
        from simworld.training.datamodule import EmulatorSequences
...
def _load_model(cfg: SimWorldConfig) -> tuple[Any, dict[str, Any]] | None:
...
def _pareto_points(cfg: SimWorldConfig) -> Any | None:
...
def main() -> None:  # pragma: no cover - exercised via `streamlit run`, not pytest
    ...
    from simworld.abm.policies import STATIC_POLICIES
    from simworld.rules import PolicyLevers, backfire
    from simworld.training.datamodule import aggregate_to_outcome

    st.set_page_config(page_title="SimWorld operator dashboard", layout="wide")
    st.title("SimWorld — policy operator dashboard")
    ...
    from simworld.visualization.interactive import network_diffusion_figure
